...
# Run live/networked chunks only when building pkgdown (or when explicitly forced).
live_eval <- identical(tolower(Sys.getenv("IN_PKGDOWN", "false")), "true") ||
    identical(tolower(Sys.getenv("MONARCHR_VIGNETTE_EVAL", "false")), "true")

knitr::opts_chunk$set(eval = live_eval, echo = TRUE, fig.width = 10, message = FALSE, warning = FALSE)
```
...
`Cystic fibrosis`. In the [Monarch Initiative](https://monarchinitiative.org) KG, this
information is stored in a *directed, labeled property graph*, with properties
attached to the entities (nodes) and relationships (edges). The [KGX format](https://github.com/biolink/kgx) used by 
...
genes associated with those. (You an use the [Monarch Initiative](https://monarchinitiative.org) website to
...
- Graph algorithms: 
    - Various: [`igraph`](https://r.igraph.org/reference/index.html), [`migraph`](https://CRAN.R-project.org/package=migraph), [`netrankr`](https://github.com/schochastics/netrankr/)
    - Matching: [`iGraphMatch`](https://CRAN.R-project.org/package=iGraphMatch)
    - Graph neural networks: [`spinner`](https://rpubs.com/giancarlo_vercellino/spinner)
    - Clustering: [`clustAnalytics`](https://CRAN.R-project.org/package=clustAnalytics)
    - Community detection: [`leidenAlg`](https://github.com/kharchenkolab/leidenAlg)
    - Link prediction: [`linkprediction`](https://CRAN.R-project.org/package=linkprediction)
- Other:
    - Particle/force-based simulation: [`particles`](https://github.com/thomasp85/particles)
    - ... others in the reverse imports for [`igraph`](https://CRAN.R-project.org/package=igraph)

# Engines

Two kinds of engines are supported, providing largely the same functionality: one supporting access to [Neo4j graph databases](https://neo4j.com/), and another supporting file-based access to TSV-formatted `.tar.gz` KGs hosted at [kghub.io](https://kghub.io). Regardless of source, KGs must conform to the [KGX specification](https://github.com/biolink/kgx).

The most accessible engine provided by `monarchr` is the `monarch_engine()`, designed specifically to access the cloud-hosted [Monarch Initiative](https://monarchinitiative.org) KG.
