...
test_that("example_graph for neo4j engine", {
    ...
    # expect_true(any("biolink:GenomicEntity" %in_list%
    # nodes(sample)$category)) expect_true(any("biolink:GeneOrGeneProduct"
    # %in_list% nodes(sample)$category))
    expect_true(any("biolink:SequenceVariant" %in_list% nodes(sample)$category))
    # expect_true(any("biolink:OntologyClass" %in_list%
    # nodes(sample)$category)) expect_true(any("biolink:PhysicalEssence"
    # %in_list% nodes(sample)$category))
