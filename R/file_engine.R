...
file_engine <- function(filename, preferences = NULL, ...) {
    ...
    # a .tar.gz file should have a *_nodes.tsv and *_edges.tsv file we should
    # check for these files and load them into the engine

    # if the file is a URL, download it save it in the users' current working
    # directory
    ...
    # suppresses non-fatal warnings; it appears that many kgs in kghub have
    # more column headers than columns which causes vroom to issue a warning
    ...
    # although we read category in as a character vector, it should be a list
    # column if there are multiple categories for a node, they will be
    # separated with |
    ...
    # let's also look for any other columns that are list columns, by seeing if
    # they contain | characters we'll split these columns into list columns
    list_cols <- names(nodes)[unlist(lapply(nodes, function(x) any(grepl("\\|", x))))]
    # drop the 'description', 'name', and 'id' columns though, those should
    # never be lists
