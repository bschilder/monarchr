#' Create a knowledge graph engine object backed by the public Monarch Neo4j
#' instance
#'
#' Creates a knowledge graph engine backed by the publicly hosted Monarch Neo4j
#' database, used to fetch nodes and edges from the database as local
#' graph objects.
#'
...
#'
#' @param url (Optional) May be specified to override the default Monarch Neo4j
#'            URL. If given a vector, each will be tried in sequence; if a URL
#'            times out (see timeout) or fails, the next is tried.
#' @param api_url (Optional) May be specified to override the default Monarch
#'                API URL (specifying the location of the `/search` endpoint
#'                used by `search_nodes()`).
