...
#'
#' This function fetches nodes (and no edges) from a knowledge graph engine
#' based on a set of
...
#'
#' - Matching node properties with boolean operators, e.g. `in_taxon_label ==
#' "Homo sapiens"`.
#' - Matching multi-valued properties with `%in_list%`, e.g. `"biolink:Gene"
#' %in_list% category`. NOTE: using `%in_list%` against vector queries, e.g.
#' `in_taxon_label %in_list% c("Homo sapiens", "Mus musculus")` is *not*
#' supported. Nor does `%in_list%` support multi-valued left hand sides;
#' `c("biolink:Disease", "biolink:Gene") %in_list% category` will not work.
#' - Boolean connectives with `|`, `&`, and `!`, e.g. `in_taxon_label == "Homo
#' sapiens" | "biolink:Gene" %in_list% category`.
#'
#' If more than one condition parameter is specified, they are combined with
#' `&`; for example,
#' `fetch_nodes(engine, in_taxon_label == "Homo sapiens", "biolink:Gene"
#' %in_list% category)` is equivalent to
#' `fetch_nodes(engine, in_taxon_label == "Homo sapiens" & "biolink:Gene"
#' %in_list% category)`.
#' @param engine A graph engine object
#' @param ... A set of conditions identifying the nodes to fetch, only used if
#'            query_ids is NULL
#' @param query_ids A character vector of identifiers to fetch
#' @param limit An integer specifying the maximum number of nodes to fetch.
