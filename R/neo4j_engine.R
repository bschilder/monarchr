...
#'
#' Creates a knowledge graph engine backed by a neo4j database, from a URL and
#' optional username and password. Knowledge graph "engines"
#' are objects that store information about how to connect to a (potentially
#' large) knowledge graph, and can be used to fetch nodes and edges from the
#' database as local
#' graph objects.
#'
...
#'
#' For `neo4j_engine()`s, preferences are also used to set the node properties
#' to search when using `search_nodes()`, defaulting to regex-based searches on
#' id, name, and description. (The `monarch_engine()` is a type
#' of `neo4j_engine()` with the URL set to the Monarch Neo4j instance, and
#' overrides `search_nodes()` to use the Monarch search API, see
#' `monarch_engine()` for details).
#'
#' @param url A character string indicating the URL of the neo4j database. If
#'            given a vector, each will be tried in sequence; if a URL times
#'            out (see timeout) or fails, the next is tried.
...
#' @param cache Whether to cache query results in memory for the length of the
#'              R session.
