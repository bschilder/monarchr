...
#' the given engine, and returns the result as a data frame.
#'
#' @param engine A neo4j_engine() or derivative providing access to a Neo4j
#'               database.
#' @param query A string representing the Cypher query, which should return a
#'              table. Multiple queries may be passed as a vector; if so,
#'              Neo2R::multicypher if used and the result is returned as a list
#'              of data frames.
#' @param parameters A list of parameters for the Cypher query, if required.
#' @param ... Additional arguments passed to the function.
#' @return The result of the Cypher query as a data frame, or a list of data
#'         frames if multiple queries are passed.
#' @export
